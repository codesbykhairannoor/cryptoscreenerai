import os
import sys
import pandas as pd
import time
from forex_executor import ForexExecutor

class WinRateOptimizer(ForexExecutor):
    def __init__(self, data):
        super().__init__()
        self.all_data = data
        self.current_idx = {tf: 0 for tf in data}
        self._dxy_cache = {"change": 0.0, "trend": "NEUTRAL", "ts": 0}

    def _get_dxy_context(self): return self._dxy_cache
    def _get_gold_orderbook(self): return {"obi": 0.0, "whale": "NORMAL"}
    def _get_gold_whale_trades(self): return "NORMAL"
    def get_account_information(self): return {"balance": 1000, "equity": 1000}

    def get_candles(self, symbol=None, timeframe="30m", limit=100):
        df = self.all_data.get(timeframe)
        if df is None or df.empty: return []
        idx = self.current_idx[timeframe]
        return df.iloc[max(0, idx-limit+1):idx+1].to_dict('records')

    def run_sim(self, tf, min_score, sl_p, tp_p, mtf_filter=True):
        df_main = self.all_data[tf]
        self.current_idx = {t: 0 for t in self.all_data}
        trades = []
        active_trade = None
        
        warmup = 100
        for i in range(warmup, len(df_main)):
            current_time = df_main.iloc[i]['dt']
            for t in self.all_data:
                tf_df = self.all_data[t]
                sync_idx = tf_df[tf_df['dt'] <= current_time].index.max()
                self.current_idx[t] = int(sync_idx) if not pd.isna(sync_idx) else 0
            
            if active_trade:
                high, low = df_main.iloc[i]['high'], df_main.iloc[i]['low']
                if (active_trade['side'] == 'buy' and low <= active_trade['sl']) or \
                   (active_trade['side'] == 'sell' and high >= active_trade['sl']):
                    trades.append(0)
                    active_trade = None
                elif (active_trade['side'] == 'buy' and high >= active_trade['tp']) or \
                     (active_trade['side'] == 'sell' and low <= active_trade['tp']):
                    trades.append(1)
                    active_trade = None
                continue

            ind = self._calc_indicators(timeframe=tf)
            if not ind or 'rsi' not in ind: continue
            
            buy_s = self._score_setup(ind, "buy", 2.5)
            sell_s = self._score_setup(ind, "sell", 2.5)
            
            side = None
            if buy_s >= sell_s and buy_s >= min_score: side = "buy"
            elif sell_s > buy_s and sell_s >= min_score: side = "sell"
            
            if side:
                pass_filter = True
                if mtf_filter:
                    mtf = self._get_mtf_confluence(side)
                    if mtf["aligned_count"] < 2: pass_filter = False
                
                if pass_filter:
                    entry_p = df_main.iloc[i]['close']
                    active_trade = {
                        'side': side,
                        'sl': entry_p - sl_p*0.1 if side == 'buy' else entry_p + sl_p*0.1,
                        'tp': entry_p + tp_p*0.1 if side == 'buy' else entry_p - tp_p*0.1
                    }

        wins = sum(trades)
        total = len(trades)
        wr = (wins/total*100) if total > 0 else 0
        pnl = (wins * (tp_p/sl_p)) - (total - wins)
        return wr, total, pnl

def optimize_all():
    print("Loading data for Multi-TF Optimization...", flush=True)
    fx = ForexExecutor()
    data = {}
    for tf in ["1m", "5m", "15m", "30m", "1h", "4h"]:
        print(f"  Fetching {tf}...", flush=True)
        c = fx.get_candles(timeframe=tf, limit=500)
        df = pd.DataFrame(c)
        df['dt'] = pd.to_datetime(df['time'])
        data[tf] = df
    
    opt = WinRateOptimizer(data)
    results = []
    
    # Test space
    timeframes = ["1m", "5m", "15m"]
    scores = [45, 60]
    sl_tp_pairs = [(20, 40), (30, 60), (40, 60)] # RR 2:1 and 1.5:1
    
    print("\nStarting Multi-TF Analysis...", flush=True)
    for tf in timeframes:
        for s in scores:
            for sl, tp in sl_tp_pairs:
                wr, count, pnl = opt.run_sim(tf, s, sl, tp)
                results.append({'tf': tf, 'score': s, 'sl': sl, 'tp': tp, 'wr': wr, 'count': count, 'pnl': pnl})
                print(f"  [{tf}] S:{s} SL:{sl} TP:{tp} -> WR:{wr:.1f}% Trades:{count}", flush=True)

    df_res = pd.DataFrame(results).sort_values(by='wr', ascending=False)
    print("\nTOP 5 STRATEGIES BY WIN RATE:")
    print(df_res.head(5))
    
    df_pnl = pd.DataFrame(results).sort_values(by='pnl', ascending=False)
    print("\nTOP 5 STRATEGIES BY PnL:")
    print(df_pnl.head(5))

if __name__ == "__main__":
    optimize_all()
