...
def forensic_audit_with_sync():
    print("="*60)
    print("SYNCED BITGET V2 FORENSIC AUDIT (Time Offset: +157s)")
    print("="*60)
    ...
    # Path v2 yang benar untuk history: /api/v2/mix/order/history
    res = v2_request("GET", "/api/v2/mix/order/history", "productType=USDT-FUTURES")
    if res.get('code') == '00000':
        data = res.get('data', [])[:10]
        for o in data:
            print(f"    - [{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(int(o['cTime'])/1000 + 7*3600))}] {o['symbol']} {o['side']} {o['state']} | Vol: {o['size']} | Comment: {o.get('orderComment', 'NONE')} | ID: {o['orderId']}")
    else:
        print(f"    Failed: {res}")
