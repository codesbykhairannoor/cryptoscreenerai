...
def evaluate(be_trig, be_lock, l1_trig, l1_lock, to_h, sl_pct):
    balance = 100.00
    peak_balance = 100.00
    max_dd = 0.0
    pos = None
...
results = []
for be_trig in [2.5, 3.5, 4.5, 5.5]:
    for be_lock in [0.4, 0.8, 1.2]:
        for l1_t in [7.0, 9.0, 11.0]:
            for l1_l in [4.0, 5.5, 7.0]:
                for to_h in [4, 6, 8, 12]:
                    for sl in [2.0, 2.5, 3.0]:
                        bal, wr, pf, dd, nt = evaluate(be_trig, be_lock, l1_t, l1_l, to_h, sl)
                        results.append((bal, wr, pf, dd, nt, be_trig, be_lock, l1_t, l1_l, to_h, sl))

results.sort(key=lambda x: x[0], reverse=True)
print("=== TOP 10 BALANCES ON CLEAN DATASET ===")
for r in results[:10]:
    print(f"Final: ${r[0]:.2f} (+{r[0]-100:.1f}%) | WR: {r[1]:.1f}% | PF: {r[2]:.2f} | DD: {r[3]:.1f}% | Tr: {r[4]} | BE:{r[5]}%->{r[6]}% | L1:{r[7]}%->{r[8]}% | TO:{r[9]}h SL:{r[10]}%", flush=True)
