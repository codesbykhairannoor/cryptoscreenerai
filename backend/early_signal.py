...
def _fetch_dex_trending() -> list:
    """
    Fetch trending pairs dari DexScreener.
    Filter: volume 5m tinggi, liquidity cukup, baru listing.
    """
    alerts = []
    try:
        # DexScreener trending pairs (free, no key needed)
        r = requests.get(
    ...
    try:
        # DexScreener latest pairs — koin yang baru listing
        for chain in ["solana", "base"]:
            ...
            for p in data[:100]:
                ...
                if (vol_5m >= DEX_MIN_VOLUME_5M and
                    ...
                    price_change_1h < 50 and   # Belum pump >50%
                    tx_count_5m >= 20):         # Aktif diperdagangkan
    ...
    while True:
        try:
            ...
            if alerts:
                ...
                # Log top DEX alerts
                new_pairs = [a for a in alerts if a.get("source") == "dexscreener_new"]
                if new_pairs:
                    # Sort by volume 5m
...
def get_early_signals() -> dict:
    ...
    return {
        ...
        "dex_alerts": [a for a in dex_alerts if a.get("source") == "dexscreener_new"][:10],
